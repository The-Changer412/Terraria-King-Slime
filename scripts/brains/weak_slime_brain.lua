...
--create the brain
local weak_slime_brain = Class(Brain, function(self, inst)
...
function weak_slime_brain:OnStart()
    ...
    local root = PriorityNode(
    {
        ...
        WhileNode(function()
            ...
            if self._state ~= "dead" then
                ...
                --make the weak slime always chase after the player
                if self._is_jump_cooldown == false then
                    if string.match(tostring(self._closest_player.components.inventory:FindItem(function(testitem) return testitem:HasTag("royal_gel") end)), "royal_gel") then
                    else
